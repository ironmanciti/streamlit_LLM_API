...
def getLLMResponse(user_input, age_option, tasktype_option):
    ...
    prefix = """
    You are a {age_option} and you want to {tasktype_option}. Here are some examples of how you can do that:
    """
    ...
    new_prompt_template = FewShotPromptTemplate(
        ...
        input_variables=["user_input","age_option","tasktype_option"],
        example_separator="\n"
    )

    # print(new_prompt_template.format(user_input=user_input, age_option=age_option, tasktype_option=tasktype_option))
    
    response = llm(new_prompt_template.format(user_input=user_input, age_option=age_option, tasktype_option=tasktype_option))
    return response

# UI 시작   
st.set_page_config(page_title="마케팅 자동화", 
                   page_icon="🧊", 
                   layout="centered", 
                   initial_sidebar_state="collapsed")
st.header("무엇을 도와드릴까요?")

user_input = st.text_area('문장을 입력해주세요', height=275)
...
if submit:
    response = getLLMResponse(user_input, age_option, tasktype_option)
    st.write(response)
